...
-- Distribution.Simple.* or Distribution.Client.* this module only ever executes
-- the `cabal-install` binary.
--
-- You can set the VERBOSE environment variable to control the verbosity of the
...
-- | Generate a random temp directory for a cabal test case and delete it after
-- the test exitted, unsureing multiple test cases don't get confused by each
-- others output. Extend the 'TestsPaths' with the generated 'tempDir' and set
-- the 'tempBuildDir' and 'configPath' as well. 'tempBuildDir' and 'configPath' are /in/
-- 'tempDir' hence they will also be removed when withTempBuildDir returns.
withTempBuildDir :: FilePath
                    -- ^ The path to the test's cabal project, i.e. the directory
                    -- containing the @.cabal@ file. The temp directory will be a
                    -- subdirectory of this directory.
                    -> TestsPaths
                    -- ^ The record with cabal paths that will be extendended
                    -> (TestsPaths -> IO a)
                    -- ^ The action, e.g. the test case, to run
                    -- during the existance of the temp
                    -- directory, that action gets an extended
                    -- 'TestsPaths'
                    -> IO a
withTempBuildDir dir paths action = bracket create remove action
  where
    create = do
      randomSuffix <- replicateM 4 $ randomRIO ('A', 'Z')
      let tempRel = dir </> "test-tmp-" ++ randomSuffix
      createDirectory tempRel
      temp <- canonicalizePath $ tempRel
      let paths' = paths { configPath = temp </> packageTestsConfigFile
                         , tempDir = Just temp
...
cabal_configure :: TestsPaths -> FilePath -> [String] -> IO Result
cabal_configure paths dir configureArgs = do
    let args = ["configure"] ++ configureArgs
    res <- cabal paths dir args
...
cabal paths dir cabalArgs = run (Just dir) (cabalPath paths) args
  where
    args = let
      configFileArg = "--config-file=" ++ configPath paths
      buildDirArg = ("--builddir=" ++) <$> maybeToList (tempBuildDir paths)
      in
       buildDirArg ++ configFileArg : cabalArgs
